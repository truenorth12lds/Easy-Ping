...
# curl -o ping_install.sh https://raw.githubusercontent.com/truenorth12lds/easy-ping/master/install.sh && sudo bash ping_install.sh
"echo" "-e" "====================="
"echo" "-e" "Uptime ping Installer"
"echo" "-e" "====================="
"echo" "-e" "Supported OS: CentOS 7/8, Ubuntu >= 16.04 and Debian"
"echo" "-e" "---------------------------------------"
"echo" "-e" "This script is designed for Linux and basic usage."
"echo" "-e" "For advanced usage, please go to https://github.com/truenorth12lds/easy-ping/wiki/Installation"
"echo" "-e" "---------------------------------------"
"echo" "-e" ""
"echo" "-e" "Local - Install Uptime ping in your current machine with git, Node.js 14 and pm2"
"echo" "-e" "Docker - Install Uptime ping Docker container"
"echo" "-e" ""
if [ "$1" != "" ]; then
  type="$1"
else
...
if [ "$type" == "local" ]; then
  defaultInstallPath="/opt/Easy-Ping"
  ...
  mkdir -p $installPath
  cd $installPath
  git clone https://github.com/truenorth12lds/EasyPing.git .
  npm run setup
  pm2 start server/server.js --name easy-ping -- --port=$port
else
  defaultVolume="Easy-Ping"
  check=$(docker -v)
  if [ "$check" == "" ]; then
  ...
  docker run -d --restart=always -p $port:3001 -v $volume:/app/data --name easy-ping louislam/easy-ping:1
fi
